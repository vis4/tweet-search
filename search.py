...
if query == 'favs':
    print '-- favs --'
    c.execute('select user, text, tstamp, id from tweets where user != "driven_by_data" order by tstamp desc')
    out = '<h2>Favorite Tweets</h2>'
elif len(query_parts) == 2 and query_parts[0][:3].lower() in months and query_parts[1] in years:
    m = months.index(query_parts[0][:3].lower())
    c.execute('select user, text, tstamp, id from tweets where strftime("%Y-%m", tstamp) = ? order by tstamp desc', ('%s-%02d' % (query_parts[1], m),))
    out = '<h2>Tweets and Favorites from %s</h2>' % query
else:
    c.execute('select user, text, tstamp, id from tweets where text like ?  order by tstamp desc', ("%" + query + "%",))
...
for row in c:
    ...
    out += '<li><strong>' + row[0] + '</strong> <span><a href="http://twitter.com/' + row[0] + '/status/' + str(row[3]) + '">' + time + '</a></span><br />' + text + '</li>'
...
span { float: right; font-weight:300 }
